...
export const countFormatter = (value, unit = undefined) => {
    ...
    if (unit === 'cost') {
        if (Math.abs(Number(value)) >= 1.0e9) customizeCount = (Math.abs(Number(value)) / 1.0e9).toFixed(2) + 'B';
        else if (Math.abs(Number(value)) >= 1.0e6) customizeCount = (Math.abs(Number(value)) / 1.0e6).toFixed(2) + 'M';
...
// export const bannerAccessControllers = (module) => {
//     const loggedInUser = getLoggedInUser();
//     const getEditRoles = loggedInUser?.roles?.filter((val) => {
//         return allModuleRoles[module]?.allowedEditRoles.includes(val);
//     });
//     return getEditRoles?.length > 0;
// };
...
export const Tooltip = ({ label, hoverText, position = ['top left', 'bottom left'], icon = false }) => (
    <Popup
